<script>
  ...
  $: if (dialog && showInsert) dialog.showModal();

  afterUpdate(() => {
    if (!showInsert && isDeleted) {
      isDeleted = false;
      location.reload();
    }
  });
  ...
  async function handleInsertClick() {
    await apiStore.addPostajalisce(naziv, naslov, latitude, longitude);
    isDeleted = true;
  }
</script>
...
<dialog
  bind:this={dialog}
  on:close={() => (showInsert = false)}
  on:click|self={() => dialog.close()}
>
  <div on:click|stopPropagation>
    <div class="modal-header">
      <h1 style="font-size: 24px;"><i class="fas fa-plus"></i> Vnesi postajališče</h1>
    </div>
    <hr />
    
    {#if isDeleted}
        <div class="modal-content">
            <p>Postajališče je bilo uspešno vneseno v podatkovno bazo.</p>
    ...
    {:else}
        ...
        <div class="property">
            ...
            <input type="text" bind:value={naziv}  required/>
        </div>
        <div class="property">
            <label><i class="fas fa-location-arrow"></i> Naslov</label>
            <input type="text" bind:value={naslov} required/>
        </div>
        <div class="property">
            <label><i class="fas fa-map-pin"></i> Latitude</label>
            <input type="text" bind:value={latitude} required/>
        </div>
        <div class="property">
            <label><i class="fas fa-map-pin"></i> Longitude</label>
            <input type="text" bind:value={longitude} required />
        </div>
        </form>
        <hr />
        <div class="btn-container">
        <button class="btn-uredi" on:click={handleInsertClick}>Vnesi postajališče</button>
