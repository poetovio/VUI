<script>
  ...
  async function onDelete() {
    await apiStore.deleteIzposoja(deleteContent.id);
    isDeleted = true;
  }
...
>
  <div on:click|stopPropagation>
    <div class="modal-header">
      <h1 style="font-size: 24px;"><i class="fas fa-trash"></i> Izbris izposoje</h1>
    </div>
    <hr />
    {#if showDelete}
      <div class="modal-content">
        {#if isDeleted}
          <p>Izposoja je bila uspešno izbrisana iz podatkovne baze.</p>
          <button class="btn-close-modal" on:click={() => dialog.close()}>Zapri</button>
        {:else}
          <p class="delete-question">
            Ali hočete izbrisati to izposojo iz podatkovne baze?
          </p>
