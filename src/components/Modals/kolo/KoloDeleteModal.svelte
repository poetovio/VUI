<script>
  ...
  async function onDelete() {
    ...
    await apiStore.deleteKolo(deleteContent.id);
    isDeleted = true;
  }
...
>
  <div on:click|stopPropagation>
    <div class="modal-header">
      <h1 style="font-size: 24px;"><i class="fas fa-trash"></i> Izbris kolesa</h1>
    </div>
    <hr />
    {#if showDelete}
      <div class="modal-content">
        {#if isDeleted}
          <p>Kolo je bilo uspešno izbrisano iz podatkovne baze.</p>
          <button class="btn-close-modal" on:click={() => dialog.close()}>Zapri</button>
        {:else}
          <p class="delete-question">
            Ali hočete izbrisati to kolo iz podatkovne baze?
          </p>
