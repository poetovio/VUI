<script>
  ...
  async function handleInsertClick() {
    await apiStore.addKolo(serijskaStevilka);
    isDeleted = true;
  }
...
>
  <div on:click|stopPropagation>
    <div class="modal-header">
      <h1 style="font-size: 24px;"><i class="fas fa-plus"></i> Vnesi novo kolo</h1>
    </div>
    <hr />
    {#if isDeleted}
      <div class="modal-content">
        <p>Kolo je bilo uspešno vneseno v podatkovno bazo.</p>
    ...
    {:else}
      <form class="modal-content">
        <div class="property">
          ...
          <input type="text" bind:value={serijskaStevilka} required />
        </div>
      </form>
    <hr />
    <div class="btn-container">
      <button class="btn-uredi" on:click={handleInsertClick}>Vnesi kolo</button>
