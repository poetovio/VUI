<script>
  ...
  $: if (dialog && showChange) {
    dialog.showModal();
    serijskaStevilka = urediContent.serijska_stevilka;
  }
  ...
  async function handleUrediClick() {
    await apiStore.updateKolo(urediContent.id, document.getElementById('serija').value);
    isDeleted = true;
  }
...
>
  <div on:click|stopPropagation>
    <div class="modal-header">
      <h1 style="font-size: 24px;"><i class="fas fa-wrench"></i> Uredi kolo</h1>
    </div>
    <hr />

    {#if isDeleted}
      <div class="modal-content">
        <p>Kolo je bilo uspešno spremenjeno.</p>
